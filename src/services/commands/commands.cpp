...
void require_parameters(string cmd){
    if (cmd=="") throw Exception(ERR_INVALID_ARGS);
}
void require_no_parameters(string cmd){
    if (cmd!="") throw Exception(ERR_INVALID_ARGS);
}
...
string cmd_ls(string cmd, unsigned int){
...
string cmd_rm(string cmd, unsigned int){
    cmd = remove_spaces(cmd);
    require_parameters(cmd);
    check_filename(cmd);
